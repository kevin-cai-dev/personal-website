                ...
                {exploreLinksInfo.map((info, index) => {
                    return (
                        <Link href={info.href} key={`${info.name} ${index}`}>
                            <div
                                className="h-24 grow basis-full bg-base-100 bg-cover bg-no-repeat transition hover:scale-105 xs:h-36 xs:basis-5/12 md:h-48 lg:h-64"
                                style={{
                                    backgroundImage: `url(${info.imgPath})`,
                                }}
                            >
                                <div className="flex h-full w-full cursor-pointer justify-center backdrop-blur-sm backdrop-brightness-50">
